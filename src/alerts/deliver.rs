...
pub trait Notifier: Send + Sync {
    /// Post `text` to the Attacca session that set the account's alerts.
...
impl Notifier for AttaccaNotifier {
    async fn send(&self, session: &str, text: &str) -> anyhow::Result<()> {
        ...
        let text = text.replace("{node}", &node_path(&conn));
        api.send_message(session.to_string(), text, Vec::new()).await?;
        Ok(())
    }
}
...
async fn deliver(app: Arc<App>, notifier: Arc<dyn Notifier>, account: String, alert_id: i64, text: String) {
    ...
    let Some(session) = app.store.alert_session(&account).await.ok().flatten() else {
        let why = "no Attacca conversation to deliver to: set the alert again from one";
        let _ = app.store.set_event_delivered(event, false, Some(why)).await;
        tracing::warn!(%account, "alert fired but no conversation is attached to the account");
        return;
    };
    for attempt in 0..2 {
        match notifier.send(&session, &text).await {
            Ok(()) => {
...
async fn deliver_untracked(app: Arc<App>, notifier: Arc<dyn Notifier>, account: String, text: String) {
    let Some(session) = app.store.alert_session(&account).await.ok().flatten() else { return };
